package com.example.notekotlin.ui.customviews
...
import com.example.notekotlin.extensions.getColorRes
...
class ColorPickerView : LinearLayout {
        ...
        Note.Color.values().forEach { color ->
            addView(
                    ColorCircleView(context).apply {
                        ...
                        setOnClickListener {
                            onColorClickListener(it.tag as
                                    Note.Color)
                        }
                    })
        }
    ...
    fun open() {
        animator.cancel()
        animator.setValues(PropertyValuesHolder.ofInt(HEIGHT, measuredHeight,
                desiredHeight),
